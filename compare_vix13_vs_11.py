"""
Compare V2 strategy with VIX_MIN=13 (old live setting) vs VIX_MIN=11 (new live setting).
How many extra trades would VIX_MIN=11 have unlocked, and what's the P&L impact?

Period: last 45 calendar days.
...
Fetching VIX from Angel One directly avoids it.
"""
...
bt.V2_VIX_MIN = 11  # reset to new live default
...
for day in sorted(set(base_days_by_date) | set(prop_days_by_date)):
    ...
    has_trades_11 = rp and rp.get("trades")

    flag = "  <-- unlocked by VIX_MIN=11" if (blocked_at_13 and has_trades_11) else ""
    print(f"{day:<12} {dow:<4} {b_desc:<28} {p_desc:<28}{flag}")

    if blocked_at_13 and has_trades_11:
...
for vmin in VIX_MINS:
    r = all_results[vmin]
    t = trades_of(r)
    print(f"VIX_MIN={vmin:<3}: {len(t)} trades, total P&L Rs.{sum(x['daily_pnl'] for x in r):,.0f}")

print(f"\nExtra trades from lowering 13 -> 11: {len(prop_trades) - len(base_trades)}")
print(f"Extra days unlocked (VIX 11-13 range): {len(new_trade_days)}")
