"""
Strategy 6b: Supertrend(10,3) Directional Follower + spot stop-loss --
45-day backtest. Same as supertrend_45day_backtest.py but adds a
50-point adverse spot-move stop-loss (matching the live V2 strategy's
SPOT_SL warn threshold), checked every 5-min candle while a position
is open. Purpose: test whether capping the tail-risk trades (the
strategy currently has NO stop-loss at all -- only exits on Supertrend
flip or forced EOD square-off) improves risk-adjusted return without
gutting the edge.
...
Exit  : (1) spot moves 50pts against the position -> SPOT_SL, checked
        first each candle; (2) Supertrend flips back the other way ->
        ST_FLIP; (3) forced EOD square-off at 15:15.
...
ST_MULT    = 3.0
COST_PER_TRADE = 40
SPOT_SL    = 50   # points adverse move against the position -> stop out

print(f"\nStrategy 6b - Supertrend(10,3) + {SPOT_SL}pt spot SL (90d): {START} to {END}")
...
for i in range(1, len(idx)):
    ...
    if position:
        ...
        if adverse >= SPOT_SL:
            t = close_trade(position, time_str, cl, "SPOT_SL")
            trades_by_date[position["entry_date"]].append(t)
            position = None
...
print("\n" + "=" * 60)
print(f"SUMMARY -- Supertrend(10,3) + {SPOT_SL}pt SL, last 90 days ({n_days} trading days)")
print("=" * 60)
