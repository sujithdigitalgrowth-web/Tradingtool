...
@app.route("/api/heavyweights")
def api_heavyweights():
    try:
        return jsonify(get_trader().get_heavyweights())
    except Exception as e:
        return jsonify({"signal": "SIDEWAYS / NO TREND", "tone": "flat",
                         "reason": "data unavailable", "stocks": [], "error": str(e)})
...
def api_portfolio():
    try:
        return jsonify(get_trader().get_portfolio())
    except Exception as e:
        return jsonify({"holdings": [], "totals": {}, "error": str(e)})
...
def api_portfolio_add():
    ...
    try:
        ...
        buy_date = d.get("buy_date") or None
        entry = get_trader().add_portfolio_holding(symbol, qty, buy_price, buy_date)
        return jsonify({"ok": True, "holding": entry})
    except (TypeError, ValueError) as e:
        return jsonify({"ok": False, "error": str(e) or "Enter a valid quantity and buy price."}), 400
    except Exception as e:
...
def api_portfolio_remove():
    d = request.json or {}
    try:
        get_trader().remove_portfolio_holding(str(d.get("id", "")))
...
def api_portfolio_cash():
    d = request.json or {}
    try:
        cash = get_trader().set_portfolio_cash(float(d.get("amount")))
        return jsonify({"ok": True, "cash": cash})
    except (TypeError, ValueError) as e:
        return jsonify({"ok": False, "error": str(e) or "Enter a valid amount."}), 400
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 400
...
def api_portfolio_buy():
    ...
    try:
        ...
        buy_date = d.get("buy_date") or None
        entry = get_trader().buy_holding(symbol, qty, price, buy_date)
        return jsonify({"ok": True, "holding": entry})
    except (TypeError, ValueError) as e:
        return jsonify({"ok": False, "error": str(e) or "Enter a valid quantity and price."}), 400
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 400
...
def api_portfolio_sell():
    d = request.json or {}
    try:
        result = get_trader().sell_holding(str(d.get("id", "")), float(d.get("qty")), float(d.get("price")))
...
def api_recommendations():
    try:
        return jsonify(get_trader().get_recommendations())
    except Exception as e:
...
def api_watchlist():
    try:
        return jsonify(get_trader().get_watchlist())
    except Exception as e:
...
def api_watchlist_add():
    d = request.json or {}
    try:
        entry = get_trader().add_watchlist_symbol(str(d.get("symbol", "")))
        return jsonify({"ok": True, "watch": entry})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 400
...
def api_watchlist_remove():
    d = request.json or {}
    try:
        get_trader().remove_watchlist_symbol(str(d.get("id", "")))
        return jsonify({"ok": True})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 400

@app.route("/api/watchlist/fundamentals", methods=["POST"])
def api_watchlist_fundamentals():
    d = request.json or {}
    try:
        symbol = str(d.get("symbol", ""))
        fundamentals = d.get("fundamentals") or {}
        entry = get_trader().set_watchlist_fundamentals(symbol, fundamentals)
        return jsonify({"ok": True, "watch": entry})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 400
...
<style>
  ...
  #holding-modal-bg{display:none;position:fixed;inset:0;background:rgba(30,27,46,.5);z-index:50;align-items:center;justify-content:center}
  #holding-modal-bg.open{display:flex}
  #watch-modal-bg{display:none;position:fixed;inset:0;background:rgba(30,27,46,.5);z-index:50;align-items:center;justify-content:center}
  #watch-modal-bg.open{display:flex}
  #sell-modal-bg{display:none;position:fixed;inset:0;background:rgba(30,27,46,.5);z-index:50;align-items:center;justify-content:center}
  #sell-modal-bg.open{display:flex}
  #funds-modal-bg{display:none;position:fixed;inset:0;background:rgba(30,27,46,.5);z-index:50;align-items:center;justify-content:center}
  #funds-modal-bg.open{display:flex}
...
<div class="flex px-6 pt-3 border-b border-violet-100 bg-white/60">
  ...
  <button onclick="switchTab('heavyweights')" id="tab-heavyweights" class="tab-i  px-5 py-2 text-sm font-semibold">Heavyweights</button>
  <button onclick="switchTab('portfolio')" id="tab-portfolio" class="tab-i  px-5 py-2 text-sm font-semibold">My Portfolio</button>
</div>
...
<!-- ══════════════ HEAVYWEIGHTS TAB ══════════════ -->
<div id="pane-heavyweights" class="p-5 hidden space-y-4">
  <div class="flex items-center justify-between px-1">
    <p class="text-xs text-gray-400 uppercase tracking-widest font-semibold">Nifty 50 &middot; Full Weighted Constituents</p>
    <p id="hw-updated" class="text-xs text-gray-400">Last updated —</p>
  </div>

  <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
    <div id="hw-signal-card" class="card p-5 lg:col-span-2">
      <p class="text-xs text-gray-400 uppercase tracking-widest font-semibold">Nifty Trend Signal</p>
      <p id="hw-signal" class="text-2xl font-extrabold mt-1">—</p>
      <p id="hw-reason" class="text-sm text-gray-500 mt-1"></p>
      <div class="mt-3">
        <div class="w-full h-2.5 rounded-full bg-red-200 overflow-hidden flex">
          <div id="hw-breadth-pos" class="h-full bg-green-500" style="width:0%"></div>
        </div>
        <div class="flex justify-between text-xs text-gray-400 mt-1">
          <span id="hw-breadth-pos-label">0% weight up</span>
          <span id="hw-breadth-neg-label">0% weight down</span>
        </div>
      </div>
    </div>
    <div class="card p-5">
      <p class="text-xs text-gray-400 uppercase tracking-widest font-semibold">NIFTY 50 Index</p>
      <p id="hw-idx-ltp" class="text-2xl font-extrabold text-gray-900 mt-1">—</p>
      <p id="hw-idx-chg" class="text-xs font-semibold mt-1">—</p>
      <p class="text-xs text-gray-400 mt-3">Weighted basket implies</p>
      <p id="hw-basket-chg" class="text-sm font-bold">—</p>
    </div>
  </div>
  ...
  <div class="card overflow-hidden">
    <div class="max-h-[600px] overflow-y-auto">
      <table class="w-full text-sm">
        <thead class="bg-gray-50 text-gray-400 text-xs uppercase tracking-wide sticky top-0">
          <tr>
            <th class="text-left font-semibold px-4 py-2.5">Stock</th>
            <th class="text-right font-semibold px-4 py-2.5">Weight</th>
            <th class="text-right font-semibold px-4 py-2.5">LTP</th>
            <th class="text-right font-semibold px-4 py-2.5">Chg %</th>
            <th class="text-right font-semibold px-4 py-2.5">Volume</th>
            <th class="text-right font-semibold px-4 py-2.5">20D Avg Vol</th>
            <th class="text-right font-semibold px-4 py-2.5">Vol x</th>
            <th class="text-center font-semibold px-4 py-2.5">State</th>
          </tr>
        </thead>
  ...
  <p class="text-xs text-gray-400 px-1">
    🟩 &ge;80% of index weight up on &gt;1.2x volume = BULLISH BOOM &middot;
    🟨 &ge;60% up but dry volume = FALSE RALLY/WEAK &middot;
    🟥 &ge;60% of index weight down on &gt;1.2x volume = BEARISH DRAG &middot;
    otherwise SIDEWAYS/NO TREND
  </p>
</div><!-- /pane-heavyweights -->

<!-- ══════════════ MY PORTFOLIO TAB ══════════════ -->
<div id="pane-portfolio" class="p-5 hidden grid grid-cols-1 lg:grid-cols-4 gap-4 items-start">

  <!-- ═══ LEFT: Watchlist ═══ -->
  <div class="lg:col-span-1 card p-4">
    <div class="flex items-center justify-between mb-1">
      <p class="text-xs text-gray-400 uppercase tracking-widest font-semibold">Watchlist</p>
      <button onclick="openWatchModal()"
        class="text-xs font-bold px-2.5 py-1 rounded-full bg-purple-100 text-purple-700 hover:bg-purple-200 transition">
        + Add
      </button>
    </div>
    <p id="wl-updated" class="text-xs text-gray-400 mb-2">Last updated —</p>
    <div id="wl-list" class="space-y-1 max-h-[640px] overflow-y-auto"></div>
  </div>

  <!-- ═══ RIGHT: Portfolio ═══ -->
  <div class="lg:col-span-3 space-y-4">
    <div class="flex items-center justify-between px-1">
      <p class="text-xs text-gray-400 uppercase tracking-widest font-semibold">My Portfolio</p>
      <div class="flex items-center gap-3">
        <p id="pf-updated" class="text-xs text-gray-400">Last updated —</p>
        <button onclick="openHoldingModal()"
          class="text-xs font-bold px-3 py-1.5 rounded-full bg-purple-100 text-purple-700 hover:bg-purple-200 transition">
          + Add / Buy
        </button>
      </div>
    </div>

    <!-- ── AI Insights: reads Heavyweights (index trend) + this portfolio + watchlist ── -->
    <div class="card p-4">
      <div class="flex items-center justify-between mb-2">
        <p class="text-xs text-gray-400 uppercase tracking-widest font-semibold">AI Insights</p>
        <p class="text-xs text-gray-300">Rule-based, not financial advice</p>
      </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-5 gap-4">
      <div class="card p-4">
        <div class="flex items-center justify-between">
          <p class="text-xs text-gray-400 uppercase tracking-widest font-semibold">Funds</p>
          <button onclick="openFundsModal()" class="text-gray-300 hover:text-purple-600 transition text-xs" title="Set available funds">✎</button>
        </div>
        <p id="pf-cash" class="text-xl font-extrabold text-gray-900 mt-1">—</p>
      </div>
      <div class="card p-4">
        <p class="text-xs text-gray-400 uppercase tracking-widest font-semibold">Invested</p>
        <p id="pf-invested" class="text-xl font-extrabold text-gray-900 mt-1">—</p>
      </div>
      <div class="card p-4">
        <p class="text-xs text-gray-400 uppercase tracking-widest font-semibold">Current Value</p>
        <p id="pf-current" class="text-xl font-extrabold text-gray-900 mt-1">—</p>
      </div>
      <div class="card p-4">
        <p class="text-xs text-gray-400 uppercase tracking-widest font-semibold">Total P&amp;L</p>
        <p id="pf-pnl" class="text-xl font-extrabold mt-1">—</p>
      </div>
      <div class="card p-4">
        <p class="text-xs text-gray-400 uppercase tracking-widest font-semibold">Today's P&amp;L</p>
        <p id="pf-day-pnl" class="text-xl font-extrabold mt-1">—</p>
      </div>
    </div>
        ...
        <table class="w-full text-sm">
          <thead class="bg-gray-50 text-gray-400 text-xs uppercase tracking-wide sticky top-0">
            <tr>
              ...
              <th class="text-right font-semibold px-4 py-2.5">Buy Price</th>
              <th class="text-right font-semibold px-4 py-2.5">LTP</th>
              <th class="text-right font-semibold px-4 py-2.5">Day %</th>
              <th class="text-right font-semibold px-4 py-2.5">Invested</th>
              <th class="text-right font-semibold px-4 py-2.5">Current</th>
              <th class="text-right font-semibold px-4 py-2.5">P&amp;L</th>
              <th class="text-left font-semibold px-4 py-2.5">Insight</th>
              <th class="text-center font-semibold px-4 py-2.5"></th>
            </tr>
          </thead>
    ...
    <p class="text-xs text-gray-400 px-1">
      Rule-based hints, not advice: 🟩 up &ge;4% today = consider taking partial profit &middot;
      🟨 down &ge;4% but still net positive = potential re-entry zone &middot;
      🟥 down &ge;10% from your buy price = flagged for review.
    </p>
  </div>
</div><!-- /pane-portfolio -->

<!-- ══════════════ ADD TO WATCHLIST MODAL ══════════════ -->
<div id="watch-modal-bg" class="modal-bg">
  <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6">
    <h2 class="text-lg font-bold text-gray-900 mb-4">Add to Watchlist</h2>
    <div class="mb-5">
      <label class="text-sm font-semibold text-gray-700 block mb-1">NSE Symbol</label>
      <input id="w-symbol" type="text" placeholder="e.g. TATAMOTORS" autocapitalize="characters"
        class="w-full border border-gray-300 rounded px-3 py-2 text-sm text-gray-800 uppercase"/>
    </div>
    <p id="w-error" class="text-xs text-red-600 mb-3 hidden"></p>
    <div class="flex gap-3 justify-end">
      ...
      <button onclick="submitWatch()"
        class="px-5 py-2 rounded text-sm font-bold bg-purple-600 text-white hover:bg-purple-700 transition">
        Add
      </button>
    </div>
  </div>
...
<!-- ══════════════ ADD HOLDING MODAL ══════════════ -->
<div id="holding-modal-bg" class="modal-bg">
  <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6">
    <h2 class="text-lg font-bold text-gray-900 mb-4">Add a Holding</h2>
    <div class="space-y-4 mb-5">
      <div>
        <label class="text-sm font-semibold text-gray-700 block mb-1">NSE Symbol</label>
        <input id="h-symbol" type="text" placeholder="e.g. RELIANCE" autocapitalize="characters"
          class="w-full border border-gray-300 rounded px-3 py-2 text-sm text-gray-800 uppercase"/>
      </div>
      <div>
        <label class="text-sm font-semibold text-gray-700 block mb-1">Quantity</label>
        <input id="h-qty" type="number" min="1" step="1"
          class="w-full border border-gray-300 rounded px-3 py-2 text-sm text-gray-800"/>
      </div>
      <div>
        <label class="text-sm font-semibold text-gray-700 block mb-1">Buy Price (₹/share)</label>
        <input id="h-price" type="number" min="0.01" step="0.01"
          class="w-full border border-gray-300 rounded px-3 py-2 text-sm text-gray-800"/>
      </div>
      <div>
        <label class="text-sm font-semibold text-gray-700 block mb-1">Buy Date (optional)</label>
        <input id="h-date" type="date"
          class="w-full border border-gray-300 rounded px-3 py-2 text-sm text-gray-800"/>
      </div>
      <label class="flex items-start gap-2 cursor-pointer">
        <input type="checkbox" id="h-from-funds" class="accent-purple-600 mt-0.5"/>
        <span class="text-xs text-gray-500">
          Deduct cost from my available funds (a real "buy"). Leave unchecked if you're just
          recording a position you already hold — that won't touch your tracked funds.
        </span>
      </label>
    </div>
    <p id="h-error" class="text-xs text-red-600 mb-3 hidden"></p>
    <div class="flex gap-3 justify-end">
      <button onclick="closeHoldingModal()"
        class="px-5 py-2 rounded text-sm font-semibold border border-gray-300 text-gray-700 hover:bg-gray-50 transition">
        Cancel
      </button>
      <button onclick="submitHolding()"
        class="px-5 py-2 rounded text-sm font-bold bg-purple-600 text-white hover:bg-purple-700 transition">
        Add
      </button>
    </div>
...
<!-- ══════════════ SELL HOLDING MODAL ══════════════ -->
<div id="sell-modal-bg" class="modal-bg">
  <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6">
    <h2 class="text-lg font-bold text-gray-900 mb-1">Sell <span id="s-symbol"></span></h2>
    <p class="text-xs text-gray-400 mb-4">Proceeds are credited back to your available funds.</p>
    <div class="space-y-4 mb-5">
      <div>
        <label class="text-sm font-semibold text-gray-700 block mb-1">Quantity to Sell</label>
        <input id="s-qty" type="number" min="1" step="1"
          class="w-full border border-gray-300 rounded px-3 py-2 text-sm text-gray-800"/>
      </div>
      <div>
        <label class="text-sm font-semibold text-gray-700 block mb-1">Sell Price (₹/share)</label>
        <input id="s-price" type="number" min="0.01" step="0.01"
          class="w-full border border-gray-300 rounded px-3 py-2 text-sm text-gray-800"/>
      </div>
    </div>
    <p id="s-error" class="text-xs text-red-600 mb-3 hidden"></p>
    <div class="flex gap-3 justify-end">
...
<!-- ══════════════ SET FUNDS MODAL ══════════════ -->
<div id="funds-modal-bg" class="modal-bg">
  <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6">
    <h2 class="text-lg font-bold text-gray-900 mb-4">Set Available Funds</h2>
    <div class="mb-5">
      <label class="text-sm font-semibold text-gray-700 block mb-1">Amount (₹)</label>
      <input id="f-amount" type="number" min="0" step="0.01" placeholder="e.g. 32240"
        class="w-full border border-gray-300 rounded px-3 py-2 text-sm text-gray-800"/>
      <p class="text-xs text-gray-400 mt-1">This replaces your current tracked balance — use it to seed
        or correct your funds, not for every deposit/withdrawal.</p>
    </div>
    <p id="f-error" class="text-xs text-red-600 mb-3 hidden"></p>
    <div class="flex gap-3 justify-end">
      <button onclick="closeFundsModal()"
        class="px-5 py-2 rounded text-sm font-semibold border border-gray-300 text-gray-700 hover:bg-gray-50 transition">
        Cancel
      </button>
...
function switchTab(name){
  ['live','history','index','heavyweights','portfolio'].forEach(t=>{
  ...
  if(name==='heavyweights') loadHeavyweights();
  if(name==='portfolio'){ loadPortfolio(); loadWatchlist(); loadRecommendations(); }
}
...
// ── Heavyweights tab ───────────────────────────────────────────
// Real-time — polls a ~49-token quote cache (the full Nifty 50 weighted
// constituent set, plus the index itself) that the server keeps warm on
// the same cadence as the header VIX feed (see live_trader.py).
let hwTimer = null;
const HW_TONE = {
  bullish: {text:'text-green-700',  badge:'bg-green-100 text-green-700'},
  bearish: {text:'text-red-700',    badge:'bg-red-100 text-red-700'},
  weak:    {text:'text-amber-700',  badge:'bg-amber-100 text-amber-700'},
  flat:    {text:'text-gray-500',   badge:'bg-gray-100 text-gray-500'},
};
async function loadHeavyweights(){
  try{
    const r = await fetch('/api/heavyweights');
    const d = await r.json();
    renderHeavyweights(d);
    document.getElementById('hw-updated').textContent =
      'Refreshes every 15s during market hours · Last checked ' + (d.time || '—');
  }catch(e){ /* keep last render on transient failure */ }
  if(!hwTimer) hwTimer = setInterval(loadHeavyweights, 15000);
}
function renderHeavyweights(d){
  const tone = HW_TONE[d.tone] || HW_TONE.flat;
  document.getElementById('hw-signal').innerHTML = `<span class="${tone.text}">${d.signal || '—'}</span>`;
  document.getElementById('hw-reason').textContent = d.reason || '';

  const posPct = d.pos_weight_pct ?? 0, negPct = d.neg_weight_pct ?? 0;
  document.getElementById('hw-breadth-pos').style.width = Math.min(100, posPct) + '%';
  document.getElementById('hw-breadth-pos-label').textContent = posPct.toFixed(0) + '% weight up';
  document.getElementById('hw-breadth-neg-label').textContent = negPct.toFixed(0) + '% weight down';

  const idx = d.index || {};
  const idxUp = (idx.pct_change ?? 0) >= 0;
  document.getElementById('hw-idx-ltp').textContent =
    idx.ltp != null ? idx.ltp.toLocaleString('en-IN',{maximumFractionDigits:2}) : '—';
  document.getElementById('hw-idx-chg').innerHTML =
    idx.pct_change != null
      ? `<span class="${cls(idx.pct_change)}">${idxUp?'▲':'▼'} ${idxUp?'+':''}${idx.pct_change.toFixed(2)}%</span>`
      : '<span class="text-gray-300">—</span>';
  document.getElementById('hw-basket-chg').innerHTML =
    idx.basket_pct_change != null
      ? `<span class="${cls(idx.basket_pct_change)}">${idx.basket_pct_change>=0?'+':''}${idx.basket_pct_change.toFixed(2)}%</span>`
      : '<span class="text-gray-300">—</span>';

  const stocks = d.stocks || [];
  const tbody = document.getElementById('hw-tbody');
  if(!stocks.length){
    tbody.innerHTML = '<tr><td colspan="8" class="text-center text-sm text-gray-400 py-8">Heavyweights data unavailable — is the broker session connected?</td></tr>';
    return;
  }
  tbody.innerHTML = stocks.map(s=>{
    const up   = s.pct_change != null && s.pct_change > 0;
    const down = s.pct_change != null && s.pct_change < 0;
    const stateBadge = up ? 'bg-green-100 text-green-700' : down ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-500';
    const stateText   = up ? 'UP' : down ? 'DOWN' : 'FLAT';
    const spike = s.vol_ratio != null && s.vol_ratio > 1.2;
    const ltp   = s.ltp != null ? s.ltp.toLocaleString('en-IN',{maximumFractionDigits:2}) : '—';
    const vol   = s.volume != null ? Math.round(s.volume).toLocaleString('en-IN') : '—';
    const avgV  = s.avg_volume != null ? Math.round(s.avg_volume).toLocaleString('en-IN') : '—';
    const volX  = s.vol_ratio != null ? s.vol_ratio.toFixed(2)+'x' : '—';
    return `<tr class="border-t border-gray-100">
      <td class="px-4 py-2.5 font-semibold text-gray-800">${s.symbol}</td>
      <td class="px-4 py-2.5 text-right text-gray-500">${s.weight.toFixed(2)}%</td>
      <td class="px-4 py-2.5 text-right font-semibold text-gray-900">${ltp}</td>
      <td class="px-4 py-2.5 text-right">${pctCell(s.pct_change)}</td>
      <td class="px-4 py-2.5 text-right text-gray-600">${vol}</td>
      <td class="px-4 py-2.5 text-right text-gray-400">${avgV}</td>
      <td class="px-4 py-2.5 text-right font-semibold ${spike?'text-blue-600':'text-gray-400'}">${volX}</td>
      <td class="px-4 py-2.5 text-center"><span class="text-xs font-bold px-2.5 py-0.5 rounded-full ${stateBadge}">${stateText}</span></td>
    </tr>`;
  }).join('');
}
...
async function loadPortfolio(){
  ...
  if(!pfTimer) pfTimer = setInterval(loadPortfolio, 15000);
}
function renderPortfolio(d){
  const t = d.totals || {};
  document.getElementById('pf-cash').textContent = t.cash != null ? inr(t.cash) : '—';
  document.getElementById('pf-invested').textContent = t.invested != null ? inr(t.invested) : '—';
  document.getElementById('pf-current').textContent  = t.current_value != null ? inr(t.current_value) : '—';
  document.getElementById('pf-pnl').innerHTML = t.pnl != null
    ? `<span class="${cls(t.pnl)}">${t.pnl>=0?'+':'-'}${inr(t.pnl)}${t.pnl_pct!=null?` (${t.pnl_pct>=0?'+':''}${t.pnl_pct.toFixed(2)}%)`:''}</span>`
    : '—';
  document.getElementById('pf-day-pnl').innerHTML = t.day_pnl != null
    ? `<span class="${cls(t.day_pnl)}">${t.day_pnl>=0?'+':'-'}${inr(t.day_pnl)}</span>`
    : '—';

  const holdings = d.holdings || [];
  const tbody = document.getElementById('pf-tbody');
  if(!holdings.length){
    tbody.innerHTML = '<tr><td colspan="10" class="text-center text-sm text-gray-400 py-10">No holdings yet — click "+ Add / Buy" to start tracking.</td></tr>';
    return;
  }
  tbody.innerHTML = holdings.map(h=>{
    const ltp  = h.ltp != null ? h.ltp.toLocaleString('en-IN',{maximumFractionDigits:2}) : '—';
    const pnlHtml = h.pnl != null
      ? `<span class="${cls(h.pnl)}">${h.pnl>=0?'+':'-'}${inr(h.pnl)}<br/><span class="text-xs">${h.pnl_pct>=0?'+':''}${h.pnl_pct.toFixed(2)}%</span></span>`
      : '<span class="text-gray-300">—</span>';
    const badge = PF_TONE[h.tone] || PF_TONE.flat;
    return `<tr class="border-t border-gray-100">
      <td class="px-4 py-2.5 font-semibold text-gray-800">${h.symbol}</td>
      <td class="px-4 py-2.5 text-right text-gray-600">${h.qty}</td>
      <td class="px-4 py-2.5 text-right text-gray-600">${h.buy_price.toLocaleString('en-IN',{maximumFractionDigits:2})}</td>
      <td class="px-4 py-2.5 text-right font-semibold text-gray-900">${ltp}</td>
      <td class="px-4 py-2.5 text-right">${pctCell(h.day_change_pct)}</td>
      <td class="px-4 py-2.5 text-right text-gray-600">${inr(h.invested)}</td>
      <td class="px-4 py-2.5 text-right text-gray-600">${h.current_value!=null?inr(h.current_value):'—'}</td>
      <td class="px-4 py-2.5 text-right">${pnlHtml}</td>
      <td class="px-4 py-2.5">
        <span class="text-xs font-bold px-2.5 py-0.5 rounded-full ${badge}">${h.signal}</span>
        <p class="text-xs text-gray-400 mt-1 max-w-xs">${h.note}</p>
      </td>
      <td class="px-4 py-2.5 text-center whitespace-nowrap">
        <button onclick="openSellModal('${h.id}')" class="text-xs font-bold text-purple-600 hover:text-purple-800 transition mr-2">Sell</button>
        <button onclick="removeHolding('${h.id}')" class="text-gray-300 hover:text-red-500 transition font-bold" title="Remove">✕</button>
      </td>
    </tr>`;
  }).join('');
  pfHoldingsById = {};
  holdings.forEach(h=>{ pfHoldingsById[h.id] = h; });
}
let pfHoldingsById = {};
function openHoldingModal(){
  ...
  document.getElementById('h-error').classList.add('hidden');
  document.getElementById('holding-modal-bg').classList.add('open');
}
function closeHoldingModal(){
  document.getElementById('holding-modal-bg').classList.remove('open');
}
async function submitHolding(){
  const symbol = document.getElementById('h-symbol').value.trim().toUpperCase();
  const qty = parseFloat(document.getElementById('h-qty').value);
  const buy_price = parseFloat(document.getElementById('h-price').value);
  const buy_date = document.getElementById('h-date').value || null;
  const fromFunds = document.getElementById('h-from-funds').checked;
  const errEl = document.getElementById('h-error');
  if(!symbol || !qty || qty<=0 || !buy_price || buy_price<=0){
    errEl.textContent = 'Enter a symbol, a positive quantity, and a positive buy price.';
    errEl.classList.remove('hidden');
    return;
  }
  try{
    const url = fromFunds ? '/api/portfolio/buy' : '/api/portfolio/add';
    const body = fromFunds ? {symbol, qty, price: buy_price, buy_date} : {symbol, qty, buy_price, buy_date};
    ...
    const d = await r.json();
    if(!d.ok){
      errEl.textContent = d.error || 'Could not add that holding.';
      errEl.classList.remove('hidden');
      return;
    }
    closeHoldingModal();
...
async function removeHolding(id){
  if(!confirm('Remove this holding from your tracked portfolio? (This is a bookkeeping removal — it will NOT credit your funds. Use Sell if you actually sold it.)')) return;
  try{
    ...
    loadPortfolio();
  }catch(e){ /* ignore, table will just look stale until next manual retry */ }
}

// ── Sell modal ───────────────────────────────────────────────────
let sellHoldingId = null;
function openSellModal(id){
  const h = pfHoldingsById[id];
  if(!h) return;
  sellHoldingId = id;
  document.getElementById('s-symbol').textContent = h.symbol;
  document.getElementById('s-qty').value = h.qty;
  document.getElementById('s-qty').max = h.qty;
  document.getElementById('s-price').value = h.ltp != null ? h.ltp : '';
  document.getElementById('s-error').classList.add('hidden');
  document.getElementById('sell-modal-bg').classList.add('open');
}
function closeSellModal(){
  document.getElementById('sell-modal-bg').classList.remove('open');
}
async function submitSell(){
  ...
  if(!qty || qty<=0 || !price || price<=0){
    errEl.textContent = 'Enter a positive quantity and price.';
    errEl.classList.remove('hidden');
    return;
  }
  try{
    const r = await fetch('/api/portfolio/sell', {
      method: 'POST', headers: {'Content-Type':'application/json'},
      body: JSON.stringify({id: sellHoldingId, qty, price})
    });
    const d = await r.json();
    if(!d.ok){
      errEl.textContent = d.error || 'Could not complete that sale.';
      errEl.classList.remove('hidden');
      return;
    }
    closeSellModal();
...
function openFundsModal(){
  ...
  document.getElementById('funds-modal-bg').classList.add('open');
}
function closeFundsModal(){
  document.getElementById('funds-modal-bg').classList.remove('open');
}
async function submitFunds(){
  ...
  try{
    ...
    const d = await r.json();
    if(!d.ok){
      errEl.textContent = d.error || 'Could not update funds.';
      errEl.classList.remove('hidden');
      return;
    }
    closeFundsModal();
...
async function loadRecommendations(){
  try{
    ...
    const d = await r.json();
    renderRecommendations(d.cards || []);
  }catch(e){ /* keep last render on transient failure */ }
  if(!recTimer) recTimer = setInterval(loadRecommendations, 15000);
}
function renderRecommendations(cards){
  const list = document.getElementById('rec-list');
  if(!cards.length){
    list.innerHTML = '<p class="text-xs text-gray-400 py-2">Nothing to flag right now.</p>';
    return;
  }
  const border = {bullish:'border-green-400',bearish:'border-red-400',weak:'border-amber-400',flat:'border-gray-300'};
  list.innerHTML = cards.map(c=>`
    <div class="border-l-4 ${border[c.tone]||border.flat} bg-gray-50 rounded-r px-3 py-2">
      <p class="text-sm font-semibold text-gray-800">${c.title}</p>
      <p class="text-xs text-gray-500 mt-0.5">${c.text}</p>
    </div>`).join('');
}

// ── Watchlist (left rail on the Portfolio tab) ───────────────────
let wlTimer = null;
async function loadWatchlist(){
  try{
    const r = await fetch('/api/watchlist');
    const d = await r.json();
    renderWatchlist(d.stocks || []);
    document.getElementById('wl-updated').textContent = 'Updated ' + (d.time || '—');
  }catch(e){ /* keep last render on transient failure */ }
  if(!wlTimer) wlTimer = setInterval(loadWatchlist, 15000);
}
function renderWatchlist(stocks){
  const list = document.getElementById('wl-list');
  if(!stocks.length){
    list.innerHTML = '<p class="text-xs text-gray-400 text-center py-6">No stocks yet — click "+ Add" to track one.</p>';
    return;
  }
  list.innerHTML = stocks.map(s=>{
    ...
    const fundPanel = f
      ? `<div id="fund-${s.id}" class="hidden mt-1 mb-1 text-xs text-gray-500 grid grid-cols-2 gap-x-3 gap-y-0.5 bg-gray-50 rounded p-2">
          <div>P/E <b class="text-gray-700">${f.pe ?? '—'}</b></div>
          <div>3Y Profit <b class="${(f.profit_growth_3y||0)>=0?'text-green-600':'text-red-500'}">${f.profit_growth_3y!=null?f.profit_growth_3y+'%':'—'}</b></div>
          <div>3Y Sales <b class="text-gray-700">${f.sales_growth_3y!=null?f.sales_growth_3y+'%':'—'}</b></div>
          <div>ROE <b class="text-gray-700">${f.roe!=null?f.roe+'%':'—'}</b></div>
          <div>D/E <b class="text-gray-700">${f.debt_equity ?? '—'}</b></div>
          <div>Promoter <b class="text-gray-700">${f.promoter_holding!=null?f.promoter_holding+'%':'—'}</b></div>
          <div class="col-span-2 text-gray-300 mt-0.5">Snapshot${f.as_of?(' '+f.as_of):''} · not live</div>
        </div>`
      : `<div id="fund-${s.id}" class="hidden text-xs text-gray-300 py-1">No fundamentals saved yet.</div>`;
    return `<div class="py-1.5 px-1 rounded hover:bg-gray-50 group">
      <div class="flex items-center justify-between">
        <div>
          <p class="text-sm font-semibold text-gray-800">${s.symbol}</p>
          <p class="text-xs text-gray-400">${ltp}</p>
        </div>
        <div class="flex items-center gap-2">
          <span class="text-xs font-bold ${up?'text-green-600':down?'text-red-500':'text-gray-400'}">
            ${s.pct_change!=null?`${up?'+':''}${s.pct_change.toFixed(2)}%`:'—'}
          </span>
          <button onclick="toggleFund('${s.id}')" class="text-gray-300 hover:text-purple-600 transition text-xs" title="Fundamentals">ⓘ</button>
          <button onclick="removeWatch('${s.id}')" class="text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition font-bold text-xs" title="Remove">✕</button>
        </div>
      </div>
      ${fundPanel}
    </div>`;
...
function openWatchModal(){
  ...
  document.getElementById('watch-modal-bg').classList.add('open');
}
function closeWatchModal(){
  document.getElementById('watch-modal-bg').classList.remove('open');
}
async function submitWatch(){
  const symbol = document.getElementById('w-symbol').value.trim().toUpperCase();
  const errEl = document.getElementById('w-error');
  if(!symbol){
    errEl.textContent = 'Enter a symbol.';
    errEl.classList.remove('hidden');
    return;
  }
  try{
    ...
    const d = await r.json();
    if(!d.ok){
      errEl.textContent = d.error || 'Could not add that symbol.';
      errEl.classList.remove('hidden');
      return;
    }
    closeWatchModal();
...
async function removeWatch(id){
  try{
    ...
    loadWatchlist();
  }catch(e){ /* ignore, list will just look stale until next manual retry */ }
}
...
document.getElementById('holding-modal-bg').addEventListener('click',function(e){
  if(e.target===this) closeHoldingModal();
});
document.getElementById('watch-modal-bg').addEventListener('click',function(e){
  if(e.target===this) closeWatchModal();
});
document.getElementById('sell-modal-bg').addEventListener('click',function(e){
  if(e.target===this) closeSellModal();
});
document.getElementById('funds-modal-bg').addEventListener('click',function(e){
