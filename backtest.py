...
V2_MAX_MOVE_PCT    = 0.50   # block entry if Nifty already moved 0.5%+ from today's open in trade direction
...
def simulate_day(target_date: date,
    ...
    day_bias       = "CE" if day_open_nifty >= prev_close else "PE"
    ...
    for i in range(1, len(nifty_candles)):
        ...
        if (not position["active"]
            ...
            # All conditions: VWAP + dual EMA + volume + RSI + BNF + Supertrend + day bias + move exhaustion
            raw_buy  = (cl > vw and cl > ef and cl > es and cl > op
                        and vol_surge and rsi > V2_RSI_MIN_CE
                        and bnf_bull and st == 1
                        and day_bias == "CE"
                        and move_pct <= V2_MAX_MOVE_PCT)
