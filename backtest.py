...
V2_SL_OPTION_PCT   = 0.15   # immediate hard SL — strong close confirms reversal
V2_SL_WARN_PCT     = 0.12   # warning zone: 2 consecutive closes needed to trigger SL
...
V2_MORNING_END     = "11:30"   # extended morning window
...
# ── Bank Nifty strategy constants ────────────────────────────────
BNF_LOT_SIZE   = 15
BNF_STRIKE_INT = 100
BNF_OPTION_IV  = 0.20     # BNF options trade at higher IV than Nifty
BNF_RSI_CE     = 55       # looser — BNF moves fast, strict RSI misses entries
BNF_RSI_PE     = 45
BNF_MRN_START  = "09:30"
BNF_MRN_END    = "11:30"  # extended — BNF has a second wave 11:00-11:30
BNF_SL         = 0.20     # wider — BNF options swing harder
BNF_SL_WARN    = 0.15
BNF_TP         = 0.50
BNF_PARTIAL    = 0.25     # partial at +25%
BNF_TRAIL_T    = 0.12     # trail activates at +12%
BNF_TRAIL_F    = 0.06     # trail floor at +6%
# ─────────────────────────────────────────────────────────────────


def estimate_option_price(spot: float, days_to_expiry: float = 7.0, iv: float = None) -> float:
    t    = days_to_expiry / 365
    iv_v = iv if iv is not None else ATM_OPTION_IV
    return round(max(spot * iv_v * np.sqrt(t) * 0.4, 10.0), 2)
...
def fetch_range_data_bnf(start: date, end: date):
    """
    Fetch all data needed for BankNifty V2 backtest.
    Spot P&L: ^NSEBANK (Yahoo Finance).
    Signal source: BANKBEES.NS 5m (same ETF used in live trader).
    Confirmation: NIFTYBEES.NS 5m (cross-index, mirrors live trader).
    VIX: ^INDIAVIX daily.
    Returns (df_5m, df_1d, df_signal, df_confirm, df_vix).
    """
    ...
    if df_5m.empty:
        raise Exception(
            f"No Bank Nifty 5-min data from Yahoo Finance for {start} to {end}. "
            "Possible causes: (1) market holiday/weekend, "
            "(2) market hasn't opened yet, "
            "(3) date older than Yahoo's 58-day intraday limit."
        )
    ...
    df_signal  = _fetch_etf("BANKBEES.NS",  start, end, interval="5m")
    df_confirm = _fetch_etf("NIFTYBEES.NS", start, end, interval="5m")
    ...
    return df_5m, df_1d, df_signal, df_confirm, df_vix
...
def _trade_record_bnf(position, exit_time, exit_spot, opt_pnl_per, reason, qty=None):
    q        = qty if qty is not None else position["qty"]
    opt_exit = round(position["entry_option_price"] + opt_pnl_per, 2)
    strike   = round(position["entry_spot"] / BNF_STRIKE_INT) * BNF_STRIKE_INT
    return {
        "time"        : position["entry_time"],
        "exit_time"   : exit_time,
        "index"       : "Bank Nifty",
        "instrument"  : "BANKNIFTY",
        "symbol"      : f"BANKNIFTY {strike} {position['type']}",
        "side"        : position["type"],
        "strike"      : strike,
        "entry"       : round(position["entry_option_price"], 2),
        "exit"        : max(opt_exit, 0),
        "entry_spot"  : round(position["entry_spot"], 2),
        "exit_spot"   : round(exit_spot, 2),
        "qty_bought"  : position.get("initial_qty", position["qty"]),
        "qty"         : q,
        "pnl"         : round(opt_pnl_per * q, 2),
        "reason"      : reason,
    }
...
def _no_trade_result_bnf(target_date, df_5m_all, df_1d_all, note=""):
    """Return a zero-trade result for BNF days we deliberately skip."""
    bnf_day = df_5m_all[df_5m_all.index.date == target_date].between_time("09:15", "15:30")
    if bnf_day.empty:
        return None
    prev_rows = df_1d_all[df_1d_all.index.date < target_date]
    if prev_rows.empty:
        return None
    prev_close = float(prev_rows.iloc[-1]["Close"])
    result = _build_result(target_date, bnf_day, prev_close, 0.0,
                           float(INITIAL_BALANCE), [], 0)
...
def simulate_day_bnf(target_date: date,
                     df_5m_all:  pd.DataFrame,
                     df_1d_all:  pd.DataFrame,
                     df_signal:  pd.DataFrame = None,
                     df_confirm: pd.DataFrame = None,
                     df_vix:     pd.DataFrame = None,
                     qty:        int = None):
    """
    Simulate V2 BankNifty strategy for one trading day.
    Uses BNF-specific params: looser RSI (55/45), morning-only window (09:30–11:00),
    wider SL (-20%) / TP (+50%), partial at +25%, trail at +12%/+6%.
    Skips Wednesday (BankNifty weekly expiry).
    """
    lot_qty = qty if qty is not None else BNF_LOT_SIZE * 2

    # Wednesday skip — BNF weekly expiry
    if target_date.weekday() == 2:
        return _no_trade_result_bnf(target_date, df_5m_all, df_1d_all,
                                    note="Wednesday — BankNifty weekly expiry skip")

    # India VIX filter (shared thresholds with Nifty)
    if df_vix is not None and not df_vix.empty:
        vix_rows = df_vix[df_vix.index.date <= target_date]
        if not vix_rows.empty:
            today_vix = float(vix_rows.iloc[-1]["Close"])
            if not (V2_VIX_MIN <= today_vix <= V2_VIX_MAX):
                return _no_trade_result_bnf(
                    target_date, df_5m_all, df_1d_all,
                    note=f"VIX {today_vix:.1f} outside range {V2_VIX_MIN}-{V2_VIX_MAX} — skip"
                )

    bnf_day = df_5m_all[df_5m_all.index.date == target_date].between_time("09:15", "15:30")
    if len(bnf_day) < V2_EMA_SLOW + 2:
        return None
    ...
    def _align_etf(df_etf):
        if df_etf is None or df_etf.empty:
            return bnf_day
        d = df_etf[df_etf.index.date == target_date].between_time("09:15", "15:30")
        d = d.reindex(bnf_day.index, method="nearest", tolerance=pd.Timedelta("3min"))
        return d if (not d.empty and d["Volume"].sum() > 0) else bnf_day

    sday = _align_etf(df_signal)   # BANKBEES — signal source
    conf = _align_etf(df_confirm)  # NIFTYBEES — cross-index confirmation

    # Indicators on BANKBEES scale
    vwap_s   = _vwap(sday)
    ema_fast = sday["Close"].ewm(span=V2_EMA_FAST, adjust=False).mean()
    ema_slow = sday["Close"].ewm(span=V2_EMA_SLOW, adjust=False).mean()
    vol_ma   = sday["Volume"].rolling(20).mean()
    ...
    has_conf  = (conf is not bnf_day)
    conf_vwap = _vwap(conf) if has_conf else None
    ...
    position = {
        "active"       : False, "type": None,
        "entry_spot"   : 0.0,   "entry_option_price": 0.0,
        "entry_time"   : None,  "qty": lot_qty,
        "trail_on"     : False, "partial_done": False,
        "sl_warn_count": 0,
    }

    bnf_candles  = list(bnf_day.iterrows())
    sig_candles  = list(sday.iterrows())
    conf_candles = list(conf.iterrows()) if has_conf else None

    for i in range(1, len(bnf_candles)):
        ts, nrow = bnf_candles[i]
        _,  srow = sig_candles[i]
        time_str = ts.strftime("%H:%M")
        ...
        # NIFTYBEES alignment (cross-index confirmation)
        if has_conf and conf_vwap is not None:
            conf_cl   = float(conf_candles[i][1]["Close"])
        ...
        if time_str >= SQUAREOFF_TIME:
            if position["active"]:
                ...
                pnl    = pnl_pu * position["qty"]
                balance += pnl; daily_pnl += pnl
                trades.append(_trade_record_bnf(position, time_str, spot_cl, pnl_pu, "EOD_SQUAREOFF"))
                trade_count        += 1
                position["active"]  = False
            break
        ...
        if position["active"]:
            ...
            if not position["trail_on"] and opt_pct >= BNF_TRAIL_T:
                position["trail_on"] = True

            # Partial exit at +25% (only when holding 2+ lots)
            if (not position["partial_done"]
                    and opt_pct >= BNF_PARTIAL
                    and position["qty"] >= BNF_LOT_SIZE * 2):
                p_pnl_pu = position["entry_option_price"] * BNF_PARTIAL
                p_spot   = (position["entry_spot"] + p_pnl_pu / 0.5
                            if position["type"] == "CE"
                            else position["entry_spot"] - p_pnl_pu / 0.5)
                partial_pnl = p_pnl_pu * BNF_LOT_SIZE
                balance += partial_pnl; daily_pnl += partial_pnl
                trades.append(_trade_record_bnf(position, time_str, p_spot, p_pnl_pu,
                                                "PARTIAL_TP", qty=BNF_LOT_SIZE))
                position["qty"]         -= BNF_LOT_SIZE
                position["partial_done"] = True

            trail_floor = BNF_TRAIL_F if position["partial_done"] else 0.0

            # SL logic with 2-close confirmation
            if opt_pct <= -BNF_SL:
                hard_action = "SL"
                position["sl_warn_count"] = 0
            elif opt_pct <= -BNF_SL_WARN:
                position["sl_warn_count"] += 1
                hard_action = "SL" if position["sl_warn_count"] >= 2 else "HOLD"
            else:
                position["sl_warn_count"] = 0
                hard_action = "TARGET" if opt_pct >= BNF_TP else "HOLD"
            ...
            if exit_reason:
                if exit_reason == "TARGET":
                    e_pnl_pu = position["entry_option_price"] * BNF_TP
                    e_spot   = (position["entry_spot"] + e_pnl_pu / 0.5
                                if position["type"] == "CE"
                                else position["entry_spot"] - e_pnl_pu / 0.5)
                elif exit_reason == "SL":
                    e_pnl_pu = -position["entry_option_price"] * BNF_SL
                    e_spot   = (position["entry_spot"] + e_pnl_pu / 0.5
                                if position["type"] == "CE"
                                else position["entry_spot"] - e_pnl_pu / 0.5)
                else:
                ...
                pnl      = e_pnl_pu * position["qty"]
                balance += pnl; daily_pnl += pnl
                trades.append(_trade_record_bnf(position, time_str, e_spot, e_pnl_pu, exit_reason))
                trade_count        += 1
                position["active"]  = False
                last_signal         = None

        # Entry gate — BNF morning-only: 09:30–11:00
        in_window = BNF_MRN_START <= time_str <= BNF_MRN_END
        ...
        if (not position["active"]
            ...
            raw_buy  = (cl > vw and cl > ef and cl > es and cl > op
                        and vol_surge and rsi > BNF_RSI_CE
                        and conf_bull and st == 1)
            raw_sell = (cl < vw and cl < ef and cl < es and cl < op
                        and vol_surge and rsi < BNF_RSI_PE
                        and conf_bear and st == -1)
            ...
            if signal:
                opt_type = "CE" if signal == "BUY_CE" else "PE"
                # DTE: days until Wednesday (weekday 2)
                dte = max(1, (2 - target_date.weekday()) % 7 or 7)
                entry_opt_price = estimate_option_price(spot_cl, dte, iv=BNF_OPTION_IV)
                if entry_opt_price * lot_qty <= balance:
                    position.update({
                        "active"          : True,  "type": opt_type,
                        "entry_spot"      : spot_cl, "entry_option_price": entry_opt_price,
                        "entry_time"      : time_str, "qty": lot_qty,
                        "initial_qty"     : lot_qty,
                        "trail_on"        : False, "partial_done": False,
                        "sl_warn_count"   : 0,
                    })

    result = _build_result(target_date, bnf_day, prev_close, daily_pnl,
                           balance, trades, trade_count)
    result["instrument"] = "BANKNIFTY"
    return result


def run_range_bnf(start: date, end: date, qty: int = None) -> list:
    """Run V2 BankNifty strategy for every trading day in [start, end]."""
    lot_qty = qty if qty is not None else BNF_LOT_SIZE * 2
    print(f"Fetching Bank Nifty + BANKBEES + NIFTYBEES + VIX data {start} to {end}...")
    df_5m, df_1d, df_signal, df_confirm, df_vix = fetch_range_data_bnf(start, end)
    results = []
    current = start
    while current <= end:
        if current.weekday() < 5:
            result = simulate_day_bnf(current, df_5m, df_1d,
                                      df_signal=df_signal, df_confirm=df_confirm,
                                      df_vix=df_vix, qty=lot_qty)
            if result:
                results.append(result)
                tag    = "SKIP" if (result["trade_count"] == 0
                                    and "expiry" in " ".join(result.get("insights", []))) else ""
                status = "+" if result["daily_pnl"] >= 0 else ""
                print(f"  {current}  PnL={status}{result['daily_pnl']:,.0f}  "
                      f"Trades={result['trade_count']}  {tag}")
        current += timedelta(days=1)
